...
def test_global_fetch_and_exact_assignment(monkeypatch, tmp_path):
    ...
    assignments, best = module.assign_locals_to_globals(
        locals_, embeddings, 0.7, 2, False, "flatip", 8, 16
    )
    assert assignments == {"g1": ["a"], "g2": ["b"]}
    assert set(best) == {"a", "b"}
    assert module.assign_locals_to_globals({}, embeddings, 0.7, 2, False, "flatip", 8, 16) == ({}, {})
    assert module.assign_locals_to_globals(locals_, {}, 0.7, 2, False, "flatip", 8, 16) == ({}, {})
...
def test_update_existing_global_speaker_confirmed_and_tentative(monkeypatch, tmp_path):
                    ...
                    [
                        {
                            ...
                            "w": 2.0,
                            "me": self.me,
                            "pid": "scott" if self.me else None,
                            "lbl": "Scott" if self.me else None,
                        }
                    ]
                )
    ...
    module.update_existing_gs_with_assignments(session, {"g": ["a", "b"]}, centroids, weights, "test", 0.5)
    ...
    centroids2 = {"x": np.array([1.0, 0.0]), "y": np.array([-1.0, 0.0])}
    module.update_existing_gs_with_assignments(session, {"g": ["x", "y"]}, centroids2, {"x": 1, "y": 1}, "test", 0.9)
...
def test_write_clusters_dry_run_and_persist(monkeypatch, tmp_path):
    ...
    session = Session()
    module.write_clusters_incremental(
        Driver(session), [["a", "b"], ["c"]], scores, centroids, weights, "m", 0.8, True, False, True
    )
    assert not session.writes

    session = Session(any_me=True)
    module.write_clusters_incremental(
        Driver(session), [["a", "b"]], scores, centroids, weights, "m", 0.8, True, True, False
    )
    assert any("GlobalSpeaker" in query for query, _ in session.writes)
    assert any("s.is_me=true" in query for query, _ in session.writes)


def test_cluster_without_faiss_edge_cases(monkeypatch, tmp_path):
    module = load_module(monkeypatch, tmp_path)
