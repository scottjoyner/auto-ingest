...
def profile():
    return {
        "font_path": "/definitely/missing.ttf", "font_size": 22,
        "line_height_px": 32, "stroke_px": 1, "max_width_px": 220,
        "colors": {"text": [255,255,255,255], "stroke": [0,0,0,255], "highlight_text": [0,0,0,255]},
        "speaker_overrides": {"g1": {"font_size": 28, "colors": {"text": [1,2,3,255]}}},
    }


def test_profiles_speakers_normalization_and_colors(tmp_path):
    pp = tmp_path / "profiles.json"; pp.write_text(json.dumps({"x": profile()}))
...
def test_wrap_tags_kind_and_caption_images():
    ...
    img = Image.new("RGBA", (240, 200)); draw = ImageDraw.Draw(img)
    lines = c._wrap_text(draw, "one two three supercalifragilisticexpialidocious", font, 70)
    assert len(lines) > 2
...
def test_sentence_highlight_wordgrid_and_render_cache(tmp_path):
    ...
    key = c._cap_key(
        "sentence", sentence="cached", width=320, profile=p, speaker_label=None,
        speaker_map={},
    )
    arr = c._render_cached(key)
    assert arr.shape[1] == 320
    key2 = c._cap_key(
        "karaoke", sentence="hello world", word="world", width=320, profile=p,
        speaker_label=None, speaker_map={},
    )
    assert c._render_cached(key2).shape[1] == 320
    key3 = c._cap_key("wordgrid", words=words, active_idx=1, width=320, profile=p, speaker_map={})
...
def test_cue_timings_scrim_hud_endcard_and_kenburns():
    ...
    explicit = c._cue_word_timings("a b", 0, 1, [{"word":"a","start":0,"end":.4}, {"word":"b"}])
    assert explicit == [("a", 0.0, .4)]
    even = c._cue_word_timings("a b c", 0, .3, None)
    assert len(even) == 3 and all(end-start >= .18 for _,start,end in even)
    ...
    class Clip:
        ...
        def fl(self, fn):
            ...
            return self
        def resize(self, size): assert size == (4,4); return self
...
def test_atomic_paths_and_safe_video_write(monkeypatch, tmp_path):
    ...
    c._write_videofile_safely(Clip(), final2, fps=30, extra_ffmpeg_params=["-metadata", "x=y"])
