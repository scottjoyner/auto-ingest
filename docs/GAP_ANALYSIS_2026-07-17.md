...
| K-N9 | med | Doc-path drift: `docs/UNIFICATION.md`/`docs/KG_INGEST_DEPLOY.md` don't exist (files at repo root); `LLD.md` still describes `knowledge_map` as real | root `UNIFICATION.md`, `KG_INGEST_DEPLOY.md` | relocate/relink; doc-lint; flag `knowledge_map` as planned |
