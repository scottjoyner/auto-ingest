...
3. First live upload: ONE platform (YouTube) at 1 post, verify it lands, then
   expand per the warm-up curve. Live upload is hard-gated — it refuses unless
   `AUTO_INGEST_LIVE=1` **and** platform credentials are present (see
   `publish_guard.py`); the default is always dry-run.
4. Deepen analytics: richer per-platform insight ingestion (YT Analytics API /
   TikTok/IG insights) once live, feeding `publish metrics ingest`.

### Already wired (dev-ready, no accounts needed)

These CLI actions exist today under
`python3 -m auto_ingest.shorts.cli publish <action>` (also reachable as
`auto-ingest shorts publish <action>`):

| Action        | What it does                                                        |
|---------------|--------------------------------------------------------------------|
| `queue`       | Stage rendered shorts (Neo4j or `--disk` via `scan_rendered`).      |
| `list`        | Show staged queue items.                                            |
| `validate`    | Check queue integrity (missing files etc).                         |
| `brand-check` | Verify brand assets/manifest/handles are present + well-formed.     |
| `auth` / `tiktok` / `instagram` | Bootstrap an OAuth token (sign-in link).         |
| `upload`      | Push queued items (dry-run by default; live needs `AUTO_INGEST_LIVE=1`). |
| `schedule`    | Turn the posting calendar into queue entries (report-only; `--apply` to enqueue). |
| `metrics`     | `ingest` / `report` / `predict` the analytics + feedback loop.      |
| `ab`          | `plan` / `list` / `choose` thumbnail + title A/B variants.          |

The scheduler (`scheduling.py`, driven by `publish schedule`) already emits a
daily posting plan from `publish_schedule.json` + rendered shorts on disk; each
slot carries its `persona_source` and a `needs_render` flag.
...
- [`avatar.png`](brand/avatar.png) + [`banner_youtube.png`](brand/banner_youtube.png) +
  [`avatar_instagram.png`](brand/avatar_instagram.png) +
  [`avatar_tiktok.png`](brand/avatar_tiktok.png) — generated by
...
```
python3 -m auto_ingest.shorts.cli publish brand-check   # manifest + asset presence/dims
python3 scripts/gen_brand_assets.py --check             # assets actually use the manifest palette + handles
```
