...
All mutating operations are bounded to a single indexed job except ``reap()``,
which explicitly limits the candidate rows *before* issuing its SET.
"""
...
SET j.completed_stages = CASE
        WHEN $stage IN coalesce(j.completed_stages, [])
        THEN coalesce(j.completed_stages, [])
        ELSE coalesce(j.completed_stages, []) + $stage
    END,
...
# LIMIT must happen before SET. `RETURN count(j) LIMIT $limit` would still mutate
# every matching job and only limit the one aggregated result row.
_REAP_QUERY = """
MATCH (j:IngestJob)
WHERE coalesce(j.owner, '') <> '' AND coalesce(j.claimed_at, 0) < $expires
WITH j
...
def create_job(driver, key: str, stages: Optional[List[str]] = None) -> Dict:
    ...
    # `stages` is retained for API compatibility; the canonical schema is fixed
    # by STAGES so repeated create_job calls cannot silently change a job schema.
    now = _now_ms()
    with driver.session() as sess:
