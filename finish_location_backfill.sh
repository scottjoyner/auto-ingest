...
PW="${NEO4J_PASSWORD:-${NEO4J_PASSWORD_DEFAULT:-}}"
if [ -z "$PW" ]; then
  echo "NEO4J_PASSWORD or NEO4J_PASSWORD_DEFAULT is required" >&2
  exit 2
fi
...
while true; do
  ...
  if [ "$running" = "no" ]; then
    ...
    if [ "$stable" -ge 2 ]; then
      echo "[$(date)] phonelog_place DONE at $cur. Running enrich_scott_unify..." >>"$LOG"
