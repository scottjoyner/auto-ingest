...
class EmbeddingSpec:
    ...
    density_heatmap: bool = True  # NEW: default to density (original heatmap.py behavior)
...
def parse_yolo_csv(csv_path: str) -> pd.DataFrame:
    ...
    Handles your example: confidence column is 'cup' (a label), and classification is '79.48%'.
    """
    rows = []
    ...
    # Standardize obvious fields
    # Frame/time-ish columns
    for c in ["frame", "t", "time", "timestamp", "cls", "class"]:
        if c in df.columns:
            df[c] = pd.to_numeric(df[c], errors="coerce")

    # --- Name / label ---
    # Prefer 'name' if present. Else 'label'. Else if 'confidence' is text (like 'cup'), use that.
    name_col = None
    if "name" in df.columns:
        ...
        # if confidence column is non-numeric text, treat as label (your example)
        if df["confidence"].map(lambda x: not pd.isna(pd.to_numeric(str(x).replace("%",""), errors="coerce"))).all():
            # it's numeric/percent -> not a label
            name_col = None
        else:
            name_col = "confidence"
    elif "classification" in df.columns:
        # sometimes classification might be a string label too
        if not df["classification"].map(lambda x: str(x).strip().endswith("%") or
                                                  pd.notna(pd.to_numeric(str(x), errors="coerce"))).all():
    ...
    # --- Confidence numeric ---
    # Try columns in order: 'confidence' (as % or float), else 'classification' (as %), else default 1.0
...
def resolve_location_for_second(
    ...
    if f and plausible_fix(prev_fix, f.get("lat"), f.get("lon"), f.get("mph")):
        # compute a meaningful seconds delta from frame distance
...
# Neo4j I/O (expanded)
# =========================
# Find nearest Frame by its key property (preferred path)
# Find nearest Frame directly by Frame.key (handles string/number frames + lon/long)
# Find nearest Frame directly by Frame.key (gps required)
...
# Fallback #1: frames attached to a Clip labeled :Clip {id:<base_key>}
...
# Fallback #2: frames attached to our :DashcamClip {key:<full key with _F/_R>}
...
# ### NEW: attach to nearest PhoneLog or LocationEvent
...
# ### NEW: enrich Frame if missing lat/lon/mph
NQ_ENRICH_FRAME = """
...
# Rebuild NEXT edges across *all* seconds present for this key/view
NQ_REBUILD_NEXT = """
MATCH (e:DashcamEmbedding {key:$key, view:$view, level:'second'})
...
def find_file_keys(directory: str) -> List[str]:
    ...
    for fn in os.listdir(directory):
        if re.search(r"_YOLOv8n\.csv$", fn):
            keys.add(fn.rsplit("_YOLOv8n", 1)[0])
    return sorted([k for k in keys if os.path.exists(os.path.join(directory, f"{k}.MP4"))])
...
def get_video_meta(mp4_path: str) -> Tuple[int, int, float, float]:
    """
    Returns (img_w, img_h, fps, dur_seconds) using VideoFileClip as a fallback.
    Called only when Neo4j doesn't already have this clip's meta.
    """
...
# =========================
# Core per-key processing (UPDATED to add location)
# =========================
...
def build_vectors_for_key(
    ...
    *,
    ...
    if pre_meta is not None and all(v is not None for v in pre_meta):
        ...
        img_w, img_h, fps, dur = get_video_meta(mp4_path)
    ...
    if df.empty:
        ...
        # Return minimal structure (whitelist vs full)
    ...
    if (seconds_whitelist is not None) and (not compute_minute):
        ...
        for s in seconds_iter:
            ...
            # metadata-only at this stage; final resolution happens later
            lat = lon = mph = None
...
def process_directory(
    ...
    with driver.session() as sess:
        ...
        for key in keys:
            ...
            # If we're not forcing and DB knows duration and everything is present → skip clip
            ...
            for vview, data in views_data.items():
                ...
                if "second_vecs_by_sec" in data:
                    ...
                    # write only those seconds; no zip over all seconds
                    if spec.per_second:
                        prev_fix = None
                        prev_fix_sec = None
                        carry_secs = 10
                        ...
                        for s_idx in sorted(sec_map_vec.keys()):
                            ...
                            # (only used if we later compute minute in some other pass)
...
def main():
    ...
    density_heatmap=not args.no_density_heatmap,  # NEW
    )
    ...
    for dd in date_dirs:
        ...
        process_directory(dd, spec, grids, keep_names, id_map, include_heatmap_png=bool(args.heatmap),
