...
def ollama_chat_json(system: str, user: str, temperature: float = 0.2) -> Dict[str, Any]:
    """
    Robust JSON chat for Ollama:
    - Forces non-streaming response (stream=false) to avoid NDJSON.
    - If server still returns NDJSON, stitches lines together.
    - Strips code fences and extracts the outermost JSON object.
    Returns a Python dict ({} if nothing parseable).
    """
    payload = {
        "model": OLLAMA_MODEL,
        "messages": [
