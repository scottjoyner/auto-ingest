#!/usr/bin/env python3
"""
Full geocode pass for ALL SummaryLocationCluster nodes (not just the top N).
Created 2026-07-16 to finish covering the ~1,000 clusters the default
cluster_place_enrich.py (MIN_COUNT=30, TOP_N=60) skipped.

Difference from the default script: MIN_COUNT=1, TOP_N=2000, and it SKIPS clusters
that already have a place_name (idempotent re-run). Rate-limited to 1 req/sec per
Nominatim policy. Writes c.place_name, c.place_category, c.geocoded_at.

Run in background: ~1,000 clusters * 1.1s = ~18 min.
"""
import neo4j
import requests
import time
...
MIN_COUNT = 1    # geocode every cluster
TOP_N = 2000     # cover all 1,047 clusters
...
def main():
    ...
    WHERE c.summary_count >= $minc AND c.place_name IS NULL
    ...
    print(f"geocoding {len(clusters)} unnamed clusters", flush=True)
