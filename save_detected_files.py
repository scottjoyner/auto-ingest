    ...
    for root, dirs, files in os.walk(base_path):
        ...
        if is_valid_date_structure(temp_path):
            ...
            for x in range(len(key_list)):
                try:
                    with VideoFileClip(os.path.join(file_path, f"{key_list[x]}.MP4")) as clip:
                        ...
                        while True:
                            ...
                            if results[0].boxes.id is not None and results[0].masks is not None:
                                ...
                                #     id = int(boxes.cls.int().cpu().tolist()[0])
                                #     print(type(id))
                                #     for key in labels.keys():
                                #         print(type(key))
                                #     print(labels[id])
                                # print(frame, boxes.id, boxes.xywh.tolist()[0], boxes.xyxy.tolist()[0])
                                # Annotate each mask with its corresponding tracking ID and color
                                # for mask, track_id in zip(masks, track_ids):
                                    # print(track_id, mask)
                                # with open(os.path.join(file_path, f"{key_list[x]}_YOLOv8n.csv"), "a") as f:
                                #     f.write(f"Key,vehicle_id,xywh,xyxy,Frame\n")
                                #     for result in results:
                                #         print(f"{key_list[x]}_{frame}_{result.boxes.id.int().cpu().tolist()[0]}", ",", result.boxes.id.int().cpu().tolist()[0], ",", result.boxes.xywh.tolist()[0], ",", result.boxes.xyxy.tolist()[0], ",", frame, file=f)
                                labels = results[0].names
                                for result in results:
                                        
                                    if result.boxes.cls is not None and result.boxes.conf is not None and result.boxes.xyxy is not None:
                                        id = int(boxes.cls.int().cpu().tolist()[0])
                                        classification = labels[id]
                                        res = result.boxes.xyxy[0].int().cpu().tolist()
                                        tmp = clip.crop(x1=res[0], y1=res[1], x2= res[2], y2=res[3])
                                        tmp.save_frame(f"{image_save_location}{key_list[x]}_{frame}_{classification}_{result.boxes.id.int().cpu().tolist()[0]}.png", frame)
