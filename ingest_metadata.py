...
def list_files(directory):
    ...
    #     if not os.path.exists(f"{directory}/{filename}_F.MP4"):
    #         file_keys.remove(filename)
    #     elif os.path.exists(f"{directory}/{filename}_FR.MP4"):
    #         file_keys.remove(filename)
    ...
    for root, dirs, files in os.walk(base_path):
        ...
        if is_valid_date_structure(temp_path):
            ...
            for filename in key_list:
                ...
                print(filename)
                with VideoFileClip(os.path.join(file_path, f"{filename}_F.MP4")) as clip_f:
                ...
                with open(os.path.join(file_path,metadata_file), 'w') as f:
