    ...
    for root, dirs, files in os.walk(base_path):
        ...
        if is_valid_date_structure(temp_path):
            ...
            for x in range(len(key_list)):
                if os.path.exists(f"{transcriptions}/{key_list[x]}_{whisper_model}_transcription.txt"):
                    with open(f"{transcriptions}/{key_list[x]}_{whisper_model}_transcription.txt", 'r') as file:
