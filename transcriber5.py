...
def create_shorts_video(input_audio_path, input_video_path, output_text_path, output_whisper_dump, output_video_path, model_size="base", target_width=1080, target_height=1920):
    ...
    with VideoFileClip(vertical_video_path) as vclip:
        ...
        for segment in result["segments"]:
            ...
            # Create an image for the full sentence
            img = create_text_image(full_sentence, font_size=60, image_size=(vclip.w, 100))
            txt_clip = ImageClip(np.array(img))
            txt_clip = txt_clip.set_duration(segment["end"] - segment["start"])
            txt_clip = txt_clip.set_position(("center", 0.75 * vclip.h))
            txt_clip = txt_clip.set_start(segment["start"])

            sentence_clips.append(txt_clip)

            # Now highlight the current word during its duration
            for word_info in segment["words"]:
                word = word_info["word"]
                start_time = word_info["start"]
                end_time = word_info["end"]
                ...
                img = create_text_image(full_sentence, font_size=60, image_size=(vclip.w, 100), highlight_word=word)
                word_txt_clip = ImageClip(np.array(img))
                word_txt_clip = word_txt_clip.set_duration(end_time - start_time)
                word_txt_clip = word_txt_clip.set_position(("center", 0.75 * vclip.h))
                word_txt_clip = word_txt_clip.set_start(start_time)

                sentence_clips.append(word_txt_clip)

        # Combine video and sentence clips
        result_clip = CompositeVideoClip([vclip] + sentence_clips)
        result_clip.write_videofile(final_output_path, fps=vclip.fps, codec="libx264", audio_codec="aac")
...
def list_files(directory, whisper_model):
    file_keys = set([])
    for filename in os.listdir(directory):
        if re.search("_FR.MP4", filename):
            file_keys.add(filename.rsplit('_FR.', 1)[0])
    file_keys_copy = file_keys.copy()
    for filename in file_keys_copy:
        if (os.path.exists(f"{directory}/{filename}_short.MP4") and os.path.exists(f"{directory}/{filename}_{whisper_model}_transcription.txt")):
...
def list_directories(base_path, whisper_model):
    for root, dirs, files in os.walk(base_path):
        ...
        if is_valid_date_structure(temp_path):
            ...
            file_path = root
            key_list = list_files(file_path, whisper_model)
            ...
            for x in range(len(key_list)):
                create_shorts_video(
                    ...
                    input_video_path=f"{file_path}/{key_list[x]}_FR.MP4",
                    output_text_path=f"{file_path}/{key_list[x]}_transcription.csv",
                    output_whisper_dump=f"{file_path}/{key_list[x]}_{whisper_model}_transcription.txt",
