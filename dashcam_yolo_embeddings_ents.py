...
# Optional deep models (not required)
_CLIP_OK = False
try:
    import torch
    _CLIP_OK = hasattr(torch, "nn")
except Exception:
    _CLIP_OK = False
...
# ---------- NEW config knobs ----------
OBJ_APPEAR_BINS = (8, 8, 8)      # B,G,R 8^3 = 512 dims
OBJ_APPEAR_MAX_PATCHES = 6       # max crops sampled per object
OBJ_APPEAR_MIN_CONF = 0.25       # ignore super low-confidence boxes for appearance
OBJ_SEMANTIC_CLASSES = [
    # expand as needed; use your YOLO label space lowercased
    "person","car","truck","bus","motorbike","bicycle","traffic light","stop sign","bench","van","suv","trailer"
]
OBJ_SEMANTIC_DIM = len(OBJ_SEMANTIC_CLASSES)
OBJ_SEMANTIC_RADIUS = 0.12       # (~12% of max(img_w,img_h)) proximity for "nearby"
OBJ_SEMANTIC_DECAY = 0.9         # temporal EMA decay for co-occur counts

# Centroid series
TRAJ_STEPS = 60                  # 1/min clip @1Hz
TRAJ_KEEP_COEFFS = 16            # rFFT coefficients kept (per channel)
TRAJ_EPS = 1e-6

# Cross-clip merge thresholds (tune!)
APPEAR_COS_THRESH = 0.87
TRAJ_COS_THRESH   = 0.82
CANDIDATES_K      = 20

# Write/batch safety
TX_MAX_OBJECTS   = 64            # max objects per write tx
TX_MAX_OBS_EDGES = 256           # max OBSERVED edges per tx
...
def _safe_float_list(x: np.ndarray) -> list:
    # avoid huge lists by ensuring float32 and Python list
    return x.astype(np.float32, copy=False).tolist()

def _quantize_xyxy(b, img_w, img_h):
    # returns x1,y1,x2,y2 in absolute pixels clamped to image
    bb = to_xyxy_abs(b, img_w, img_h)
    if bb is None: return None
    x1,y1,x2,y2 = bb
    return max(0,min(img_w,x1)), max(0,min(img_h,y1)), max(0,min(img_w,x2)), max(0,min(img_h,y2))

def _centroid_norm(b, img_w, img_h):
    bb = _quantize_xyxy(b, img_w, img_h)
...
def _series_to_rfft_features(series: np.ndarray, keep: int) -> np.ndarray:
    """
    series: shape [T, C] (e.g., T=60, C=4 for cx,cy,dx,dy or cx,cy,w,h)
    returns concatenated real magnitudes of rFFT across channels (DC..keep-1)
    """
    T, C = series.shape
    feats = []
    for c in range(C):
        s = series[:, c].astype(np.float32, copy=False)
        s -= np.mean(s)
        spec = np.fft.rfft(s)  # length floor(T/2)+1
        mag = np.abs(spec)[:keep]
        feats.append(mag)
    return np.concatenate(feats, axis=0).astype(np.float32, copy=False)

def _hist_3d_bgr(patch: np.ndarray, bins=(8,8,8)) -> np.ndarray:
    # cv2 reads BGR; normalize histogram
    if patch is None or patch.size == 0:
        return np.zeros((bins[0]*bins[1]*bins[2],), dtype=np.float32)
    if cv2 is None:
        # fallback: mean/std per channel
        x = patch.reshape(-1, patch.shape[-1]).astype(np.float32)
        mu = np.mean(x, axis=0); sd = np.std(x, axis=0)
        return np.concatenate([mu, sd], axis=0).astype(np.float32)
    h = cv2.calcHist([patch],[0,1,2],None,bins,[0,256,0,256,0,256]).flatten().astype(np.float32)
    s = h.sum() + 1e-9
    h /= s
    return h

def _crop_patch(cap, frame_idx, bbox, scale=1.05):
    if cap is None or bbox is None: return None
    x1,y1,x2,y2 = bbox
    cx, cy = int((x1+x2)/2), int((y1+y2)/2)
    w, h = int((x2-x1)*scale), int((y2-y1)*scale)
    x1n, y1n = max(0, cx - w//2), max(0, cy - h//2)
    x2n, y2n = x1n + w, y1n + h
    ...
    if x2n <= x1n or y2n <= y1n: return None
    patch = frame[y1n:y2n, x1n:x2n]
...
NQ_CREATE_CONSTRAINTS = [
    ...
    """,
    """
    CREATE CONSTRAINT detected_object_id IF NOT EXISTS
    FOR (d:DetectedObject) REQUIRE d.id IS UNIQUE
    """,
    """
    CREATE INDEX detected_object_class IF NOT EXISTS
    FOR (d:DetectedObject) ON (d.class)
    """,
]
...
# ---------- NEW: Cypher for DetectedObject upsert and linking ----------

NQ_UPSERT_DETECTED_OBJECT = """
MERGE (d:DetectedObject {id:$id})
ON CREATE SET d.class=$class, d.view=$view, d.first_seen=$first_seen, d.created_at=timestamp()
SET d.last_seen=$last_seen,
    d.count = coalesce(d.count,0) + $seen_inc,
    d.traj_dim=$traj_dim, d.appearance_dim=$appearance_dim, d.semantic_dim=$semantic_dim,
    d.traj_vec=$traj_vec,
    d.appearance_vec=$appearance_vec,
    d.semantic_vec=$semantic_vec
RETURN d
"""

NQ_LINK_OBJECT_OBSERVED = """
MATCH (d:DetectedObject {id:$id})
MATCH (e:DashcamEmbedding {id:$eid})
MERGE (d)-[:OBSERVED {key:$key, second:$sec, conf:$conf}]->(e)
"""

# Small metadata edge to the clip for convenience
NQ_LINK_OBJECT_CLIP = """
MATCH (d:DetectedObject {id:$id})
MATCH (c:DashcamClip {key:$key})
MERGE (d)-[:APPEARS_IN]->(c)
"""
...
def find_merge_candidate(
    sess, *, clazz: str, view: str,
    traj_vec: np.ndarray, appearance_vec: np.ndarray,
    k: int = CANDIDATES_K
):
    """
    Try vector index on appearance first (more discriminative), then check trajectory.
    Returns (node_id or None).
    """
    # Attempt vector index query
    try:
        recs = sess.run("""
            CALL db.index.vector.queryNodes('detected_object_appear', $vec, $k)
            YIELD node, score
            WHERE node.class = $clazz AND (node.view = $view OR node.view IS NULL)
            RETURN elementId(node) AS elem_id, node.id AS id, score,
                   node.traj_vec AS traj_vec
            ORDER BY score DESC
            LIMIT $k
        """, vec=_safe_float_list(appearance_vec), k=int(k), clazz=clazz, view=view)
        best = None; best_score = -1.0
        for r in recs:
            cand_id = r["id"]; sc = float(r["score"])
            if sc < APPEAR_COS_THRESH:  # vector-index returns higher=closer (cosine sim)
                continue
            tvec = np.array(r.get("traj_vec") or [], dtype=np.float32)
            if tvec.size:
                tcos = _cosine(traj_vec, tvec)
                if tcos >= TRAJ_COS_THRESH and sc > best_score:
                    best, best_score = cand_id, sc
        return best
    except Exception as e:
        logging.info(f"[merge] vector index not available; falling back: {e}")

    # Fallback: pull at most k recent candidates by class+view (small memory!)
    recs = sess.run("""
        MATCH (d:DetectedObject {class:$clazz})
        WHERE d.view = $view OR d.view IS NULL
        RETURN d.id AS id, d.appearance_vec AS appearance_vec, d.traj_vec AS traj_vec
        ORDER BY d.last_seen DESC
        LIMIT $k
    """, clazz=clazz, view=view)
    best = None; best_score = -1.0
    for r in recs:
        avec = np.array(r.get("appearance_vec") or [], dtype=np.float32)
        if avec.size == 0: continue
        sc = _cosine(appearance_vec, avec)
        if sc < APPEAR_COS_THRESH: continue
        tvec = np.array(r.get("traj_vec") or [], dtype=np.float32)
        tcos = _cosine(traj_vec, tvec) if tvec.size else 1.0
        if tcos >= TRAJ_COS_THRESH and sc > best_score:
            best, best_score = r["id"], sc
    return best


def neo4j_create_vector_indexes(sess, traj_dim: int, appear_dim: int, sem_dim: int):
    stmts = [
        f"""
        CREATE VECTOR INDEX detected_object_traj IF NOT EXISTS
        FOR (d:DetectedObject) ON (d.traj_vec)
        OPTIONS {{
          indexConfig: {{
            `vector.dimensions`: {traj_dim},
            `vector.similarity_function`: 'COSINE'
          }}
        }}
        """,
        f"""
        CREATE VECTOR INDEX detected_object_appear IF NOT EXISTS
        FOR (d:DetectedObject) ON (d.appearance_vec)
        OPTIONS {{
          indexConfig: {{
            `vector.dimensions`: {appear_dim},
            `vector.similarity_function`: 'COSINE'
          }}
        }}
        """,
        f"""
        CREATE VECTOR INDEX detected_object_sem IF NOT EXISTS
        FOR (d:DetectedObject) ON (d.semantic_vec)
        OPTIONS {{
          indexConfig: {{
            `vector.dimensions`: {sem_dim},
            `vector.similarity_function`: 'COSINE'
          }}
        }}
        """,
    ...
    for s in stmts:
        try:
            ...
            logging.warning(f"[vector-index] skipped: {e}")

def _build_object_embeddings_for_clip(
    key: str,
    view: str,
    df: pd.DataFrame,
    img_w: int, img_h: int,
    ...
    Returns list of dicts:
    {
      'class': str,
      'view': str,
      'id_local': str,           # per-clip identifier (vehicle_id/track or synthetic)
      'traj_vec': np.ndarray,
      'appearance_vec': np.ndarray,
      'semantic_vec': np.ndarray,
      'obs': List[ (sec, emb_id, conf) ],  # for OBSERVED edges
      'first_seen': int, 'last_seen': int
    }
    """
    # 1) group detections into tracks per object
    # prefer vehicle_id column; else fallback to (frame→IoU) tracker-lite
    cols = {c.lower(): c for c in df.columns}
    vid_col = cols.get("vehicle_id")
    cls_col = "name"

    # normalize timestamps → seconds
    if "__sec__" not in df.columns:
        df = assign_seconds(df, fps=fps, dur=dur, key=key)

    # filter to classes of interest OR keep all if keep_names empty
    if keep_names:
        df = df[df.apply(lambda r: keep_detection(r, keep_names, DEFAULT_CLASS_ID_MAP), axis=1)]

    # build per-id buckets
    buckets = defaultdict(list)
    if vid_col:
        for r in df.itertuples(index=False):
            rid = getattr(r, vid_col)
            if pd.isna(rid):  # skip untracked
                continue
            buckets[str(rid)].append(r)
    else:
        # fallback: cluster by frame IoU proximity to form pseudo-ids (very light)
        active = []
        _next_id = 0
        for r in df.sort_values(by=[cols.get("frame","frame")]).itertuples(index=False):
            rowd = r._asdict() if hasattr(r, "_asdict") else r._asdict()
            bb = to_xyxy_abs(rowd, img_w, img_h)
            if bb is None: continue
            matched = False
            for i, (pid, pbb, pframe) in enumerate(list(active)):
                if abs((rowd.get("frame") or 0) - pframe) > int(max(1, round(fps/2))):
                    active.pop(i); continue
                inter = bbox_overlap_area(*bb, *pbb)
                if inter > 0:
                    matched = True
                    buckets[pid].append(r); active[i] = (pid, bb, rowd.get("frame") or 0)
                    break
            if not matched:
                pid = f"track_{_next_id}"; _next_id += 1
                buckets[pid].append(r); active.append((pid, bb, rowd.get("frame") or 0))

    # open video capture for appearance patches if possible
    cap = None
    ...
    objects = []
    # Pre-index class names for semantic vectors
    sem_index = {c:i for i,c in enumerate(OBJ_SEMANTIC_CLASSES)}

    for oid, rows in buckets.items():
        if not rows: continue
        # class: choose majority label
        labels = [str(getattr(r, cls_col, "")).lower() for r in rows if getattr(r, cls_col, None) is not None]
        clazz = max(labels, key=labels.count) if labels else "object"

        # --- trajectory series (cx,cy,w,h) & (dx,dy)
        # resample to 1Hz (by sec)
        sec_to_xy = {}
        for r in rows:
            rd = r._asdict() if hasattr(r, "_asdict") else r._asdict()
            sec = int(rd.get("__sec__") or 0)
            info = _centroid_norm(rd, img_w, img_h)
            if info:
                sec_to_xy[sec] = info  # cx,cy,w,h

        if not sec_to_xy:
            continue
        secs = sorted(sec_to_xy.keys())
        t_first, t_last = min(secs), max(secs)

        # fill a fixed-length array of TRAJ_STEPS by sampling between [t_first, t_last]
        # map seconds into 0..TRAJ_STEPS-1
        traj_raw = np.zeros((TRAJ_STEPS, 4), dtype=np.float32)
        if t_last == t_first:
            vals = np.array(sec_to_xy[t_first], dtype=np.float32)
            traj_raw[:] = vals
        else:
            for s in range(TRAJ_STEPS):
                rel = s / max(1, (TRAJ_STEPS-1))
                t = int(round(t_first + rel*(t_last - t_first)))
                traj_raw[s] = np.array(sec_to_xy.get(t, sec_to_xy[secs[min(len(secs)-1, max(0, t - t_first))]]), dtype=np.float32)

        # velocities approx
        v = np.vstack([np.zeros((1,2), dtype=np.float32), np.diff(traj_raw[:,:2], axis=0)])
        traj_feat = _series_to_rfft_features(np.hstack([traj_raw[:,:2], v]), keep=TRAJ_KEEP_COEFFS)
        traj_feat = _unit(traj_feat)

        # --- appearance vector ---
        # sample up to N patches across the time span
        frames = sorted({int(getattr(r, cols.get("frame","frame"), 0) or (int(getattr(r, "__sec__", 0))*round(fps))) for r in rows})
        sample_frames = np.linspace(0, len(frames)-1, num=min(OBJ_APPEAR_MAX_PATCHES, len(frames)), dtype=int)
        hists = []
        for idx in sample_frames:
            r = rows[idx]
            rd = r._asdict() if hasattr(r, "_asdict") else r._asdict()
            conf = float(rd.get("confidence") or 1.0)
            if conf < OBJ_APPEAR_MIN_CONF: 
                continue
            bb = _quantize_xyxy(rd, img_w, img_h)
            if cap is not None:
                patch = _crop_patch(cap, int(rd.get("frame") or 0), bb)
            else:
                patch = None
            h = _hist_3d_bgr(patch, bins=OBJ_APPEAR_BINS)
            # extra geom: aspect & area
            if bb:
                x1,y1,x2,y2 = bb
                w = (x2-x1)/max(1,img_w); hgt = (y2-y1)/max(1,img_h)
                geom = np.array([w, hgt, (w*hgt), w/(hgt+1e-9)], dtype=np.float32)
            else:
                geom = np.zeros((4,), dtype=np.float32)
            hists.append(np.concatenate([h, geom], axis=0))
        if hists:
            appear = _unit(np.mean(np.stack(hists, axis=0), axis=0))
        else:
            appear = np.zeros((OBJ_APPEAR_BINS[0]*OBJ_APPEAR_BINS[1]*OBJ_APPEAR_BINS[2] + 4,), dtype=np.float32)

        # Optional: upgrade with CLIP features if available
        # (left as future hook; keep memory light in first pass)

        # --- semantic co-occurrence vector ---
        sem = np.zeros((OBJ_SEMANTIC_DIM,), dtype=np.float32)
        # Build per-second class histogram and aggregate with EMA
        sec_groups = defaultdict(list)
        for r in rows:
            sec_groups[int(getattr(r, "__sec__", 0) or 0)].append(r)
        ema = np.zeros_like(sem)
        for s in sorted(sec_groups.keys()):
            local = np.zeros_like(sem)
            for r in df[df["__sec__"] == s].itertuples(index=False):
                label = str(getattr(r, cls_col, "")).lower()
                if label in sem_index:
                    # optionally weight by proximity
                    rd = r._asdict()
                    my = sec_to_xy.get(s)
                    his = _centroid_norm(rd, img_w, img_h)
                    if my and his:
                        dx = (his[0] - my[0]); dy = (his[1] - my[1])
                        dist = math.sqrt(dx*dx + dy*dy)
                        w = 1.0 if dist <= OBJ_SEMANTIC_RADIUS else 0.25
                    else:
                        w = 0.5
                    local[sem_index[label]] += w
            ema = OBJ_SEMANTIC_DECAY * ema + (1.0 - OBJ_SEMANTIC_DECAY) * local
        sem = _unit(ema)

        # observations mapping for edges
        obs = []
        for s in secs:
            emb_id = f"{key}|{view}|sec|{s}"
            # use the *max* confidence that second for this object
            conf_s = max(float(getattr(r, "confidence") or 1.0) for r in rows if int(getattr(r, "__sec__", 0) or 0) == s)
            obs.append((s, emb_id, conf_s))

        objects.append({
            "class": clazz,
            "view": view,
            "id_local": f"{key}:{view}:{oid}",
            "traj_vec": traj_feat,
            "appearance_vec": appear,
            "semantic_vec": sem,
            "obs": obs,
            "first_seen": int(t_first),
            "last_seen":  int(t_last),
        })
    if cap is not None:
        cap.release()
    return objects

def _write_objects_batched(sess, key: str, objects: List[dict]):
    """
    Writes objects in small batches to avoid large params allocations.
    """
    if not objects:
        return

    # dimensions (stable across all objects)
    traj_dim = int(objects[0]["traj_vec"].shape[0])
    appear_dim = int(objects[0]["appearance_vec"].shape[0])
    sem_dim = int(objects[0]["semantic_vec"].shape[0])

    # ensure clip link created once per object ID
    def upsert_chunk(chunk):
        tx = sess.begin_transaction()
        try:
            for obj in chunk:
                # Merge/attach to global object via similarity
                cand = find_merge_candidate(
                    sess, clazz=obj["class"], view=obj["view"],
                    traj_vec=obj["traj_vec"], appearance_vec=obj["appearance_vec"],
                    k=CANDIDATES_K
                )
                obj_id = cand or str(uuid.uuid4())

                tx.run(NQ_UPSERT_DETECTED_OBJECT, **{
                    "id": obj_id,
                    "class": obj["class"],
                    "view": obj["view"],
                    "first_seen": obj["first_seen"],
                    "last_seen": obj["last_seen"],
                    "seen_inc": len(obj["obs"]),
                    "traj_dim": traj_dim,
                    "appearance_dim": appear_dim,
                    "semantic_dim": sem_dim,
                    "traj_vec": _safe_float_list(obj["traj_vec"]),
                    "appearance_vec": _safe_float_list(obj["appearance_vec"]),
                    "semantic_vec": _safe_float_list(obj["semantic_vec"]),
                })
                tx.run(NQ_LINK_OBJECT_CLIP, id=obj_id, key=key)

                # OBSERVED edges (bounded per tx to avoid blowups)
                # chunk inside too if needed
                for (sec, eid, conf) in obj["obs"][:TX_MAX_OBS_EDGES]:
                    tx.run(NQ_LINK_OBJECT_OBSERVED, id=obj_id, eid=eid, key=key, sec=int(sec), conf=float(conf))
            tx.commit()
        except Exception as e:
            tx.rollback()
            logging.error(f"[tx] object batch failed: {e}")
            raise

    # outer chunking on objects
    for i in range(0, len(objects), TX_MAX_OBJECTS):
        upsert_chunk(objects[i:i+TX_MAX_OBJECTS])
...
def process_directory(
    ...
    with driver.session() as sess:
        ...
        # Create vector indexes lazily (best-effort)
        try:
            # dims: trajectory = 4 channels * TRAJ_KEEP_COEFFS*? (we used cx,cy,dx,dy = 4 channels)
            traj_dim = TRAJ_KEEP_COEFFS * 4
            appear_dim = OBJ_APPEAR_BINS[0]*OBJ_APPEAR_BINS[1]*OBJ_APPEAR_BINS[2] + 4
            sem_dim = OBJ_SEMANTIC_DIM
            neo4j_create_vector_indexes(sess, traj_dim, appear_dim, sem_dim)
        except Exception as _e:
            logging.info(f"[vector-index] skipped: {_e}")
        ...
        for key in keys:
            ...
            # ---------- NEW: object-level embeddings & merges ----------
            # Build once per view using the same df we parsed earlier.
            # Re-parse YOLO CSV quickly to get full det rows (already parsed above in build_vectors_for_key,
            # but that returns processed grids; so we parse again, cheap.)
            csv_path = os.path.join(date_dir, f"{key}_YOLOv8n.csv")
            if os.path.exists(csv_path):
                try:
                    df_det = parse_yolo_csv(csv_path)
                    if not df_det.empty:
                        # ensure seconds column
                        # (prefer metadata we already know)
                        vdata = views_data.get(vview)
                        img_w = int(vdata.get("img_w", 0) or 0)
                        img_h = int(vdata.get("img_h", 0) or 0)
                        fps_v = float(vdata.get("fps", fps_db or 30.0))
                        dur_v = float(vdata.get("dur", dur_db or 0.0))
                        mp4 = vdata.get("path") or os.path.join(date_dir, f"{key}.MP4")

                        objs = _build_object_embeddings_for_clip(
                            key=key, view=vview, df=df_det,
                            img_w=img_w, img_h=img_h,
                            fps=fps_v, dur=dur_v,
                            mp4_path=mp4,
                            keep_names=set()  # or reuse your keep_names if you want to restrict
                        )

                        # write in small batches to dodge big allocations
                        _write_objects_batched(sess, key=key, objects=objs)
                except Exception as e:
                    logging.warning(f"[objects] failed for {key}/{vview}: {e}")
