...
Usage
-----
# Ingest everything with info logs, read taxonomy, write confmats:
...
def load_transcription_json_txt(path: str) -> Optional[Dict[str, Any]]:
    """
    NEW PIPELINE: <model>_transcription.txt contains full JSON (Whisper or faster-whisper-compatible).
    Returns dict: {text, segments, language, file_started_at, file_ended_at}.
    """
...
def load_transcription_csv(path: str) -> Optional[Dict[str, Any]]:
    """
    Fallback CSV reader. Accepts flexible columns and absolute time hints.
    """
...
def load_subjects_sidecar(path: str) -> List[Dict[str, Any]]:
    """
    Your existing sidecar like: <key>_<model>_transcription_subjects.json
    Holds pre-aggregated entities structure: {"entities":[{id,text,label,count,starts,ends,avg_score}, ...]}
    Return as-is (list of dicts).
    """
    try:
        with open(path, "r", encoding="utf-8") as f:
            j = json.load(f)
...
def gliner_extract(text: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Include a broader label set; GLiNER will generalize
...
def _compile_patterns(patterns: List[str]) -> re.Pattern:
    if not patterns:
        return re.compile(r"$^", re.I)  # never matches
...
def _ensure_subject_label_vecs(batch_size: int) -> List[List[float]]:
    ...
    for lbl in SUBJECT_LABELS:
        ...
        # Simple descriptive prompt for label embedding; include synonyms to broaden
...
def classify_subjects_for_text(
    ...
    """
    Score each subject label as blend of cosine(unit,label) and regex(hit).
    Returns sorted list [(label, score), ...] filtered by threshold.
    """
...
def discover_keys() -> Dict[str, Dict[str, Any]]:
    """
    Walk SCAN_ROOTS, collect *all* json/csv/entities/subjectsJSON/rttm/media into lists per key.
    """
    with TimedStage(st_discover, detail=f"roots={len(SCAN_ROOTS)}"):
        ...
        def add(kind: str, path: str):
            ...
            if kind == "json":
                ...
                mapping[key][kind] = path  # single last wins
...
def process_key(key: str, paths: Dict[str, Any], driver, batch_size: int, dry_run: bool = False):
    ...
    # Prefer sidecar JSON if present (already aggregated)
    aggregated_entities = []
    if subjects_sidecar_path and os.path.isfile(subjects_sidecar_path):
        ...
        # raw entities from CSV if present
        ...
        # Write sidecar near source (optional)
        if aggregated_entities and src_dir_for_sidecar:
            try:
                subj_model = "best"
                ...
                with open(subjects_path, "w", encoding="utf-8") as f:
                    json.dump({"entities": aggregated_entities}, f, ensure_ascii=False, indent=2)
    ...
    if "rttm" in paths and paths["rttm"] and os.path.isfile(paths["rttm"]):
        ...
        # Link utterances to nearest segment by max overlap
        for u in utts:
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Ingest transcripts (dashcam/bodycam/audio) with best-model selection, taxonomy subjects, GLiNER, validation, and rich logging.")
    # Search mode (optional — if you want it, paste your previous vector_search body and re-enable)
    ...
    parser.add_argument("--confmat-topk", type=int, default=15, help="How many top co-occurring subject pairs to log.")
    ...
    # Write confusion matrices if requested
    if args.confmat_out:
        try:
